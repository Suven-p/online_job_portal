...
class Recommender:
    ...
    async def close(self):
        self.db.close()
        await self.academics.close()
        await self.parser.close()

    def getUserSkills(self, uid):
        cursor = self.db.cursor()
        cursor.execute(
            "SELECT name, proficiency from applicant_skills WHERE id = %s", (uid,)
        )
    ...
    def getJobSkills(self, jid):
        cursor = self.db.cursor()
        cursor.execute(
            "SELECT skillName, proficiency from skills where jobId = %s", (jid,)
        )
        return cursor.fetchall()
    ...
    async def computeRecommendation(self):
        ...
        applicantCursor.execute("SELECT id FROM applicant_data")
        applicants = [i[0] for i in applicantCursor]
        jobCursor = self.db.cursor()
        jobCursor.execute("SELECT jobId from jobs")
        jobs = [j[0] for j in jobCursor]
        await asyncio.gather(
            *[self.processTuple(aId, jId) for aId in applicants for jId in jobs]
        )
    ...
    async def calculateRankingJob(self, jId):
        ...
        applicantCursor.execute("SELECT id FROM applicant_data")
        applicants = [i[0] for i in applicantCursor]
        finalScores = await asyncio.gather(
            *[self.processTuple(aId, jId) for aId in applicants]
        )
        for index, score in enumerate(finalScores):
            if score > 0.5:
                ...
                async with self.parser.session.get(
                    "http://localhost:3500/api/verify/test", params=params
                ) as response:
